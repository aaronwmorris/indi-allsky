...
class IndiClient(PyIndi.BaseClient):
    ...
    def __init__(
        ...
        self._indi_allsky_camera_name = None
    ...
    def setCcdGain(self, new_gain):
        ...
        if indi_exec in [
            ...
            return self.ccd_device.setCcdGain(new_gain)
        elif 'indi_pylibcamera' in indi_exec:  # SPECIAL CASE
            # the exec name can have many variations
            gain_config = {
                "PROPERTIES" : {
    ...
    def generateIndiAllskyCameraName(self):
        ...
        if indi_exec in [
            ...
            try:
                ...
                logger.warning('Overriding camera name: %s', camera_name)
            except TimeOutException:
            ...
            # Add "CCD " to match the name generated by indi_asi_ccd
            if re.search(r'^ZWO (?!CCD)', camera_name):
                camera_name = camera_name[:4] + 'CCD ' + camera_name[4:]
                logger.warning('Overriding camera name: %s', camera_name)
            ...
            # Add "CCD " to match the name generated by indi_playerone_ccd
            if re.search(r'^PlayerOne (?!CCD)', camera_name):
                camera_name = camera_name[:10] + 'CCD ' + camera_name[10:]
                logger.warning('Overriding camera name: %s', camera_name)
