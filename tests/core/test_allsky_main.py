...
def test_start_capture_worker_dead_with_empty_error_queue(create_allsky_instance):
    """Test lines 424-425: queue.Empty when capture_error_q has no errors."""
    allsky = create_allsky_instance()
    mock_worker = MagicMock()
    mock_worker.is_alive.return_value = False
    allsky.capture_worker = mock_worker

    with patch('indi_allsky.capture.CaptureWorker') as mock_cw_cls:
        new_worker = MagicMock()
        mock_cw_cls.return_value = new_worker
        allsky._startCaptureWorker()
