...
def test_libcamera_mqtt_day_awb_disabled_and_sync_mode(flask_app):
    image_q = Queue()
    position_av = Array('d', [0.0] * 5)
    exposure_av = Array('f', [-1.0] * 7)
    gain_av = Array('f', [-1.0] * 10)
    binning_av = Array('i', [1] * 6)
    night_av = Array('i', [0, 0])  # Daytime
    ...
    with patch('paho.mqtt.client.Client', return_value=mock_client), \
        ...
        cam = IndiClientLibCameraMqttGeneric(config, image_q, position_av, exposure_av, gain_av, binning_av, night_av)
        cam.connectServer()
        cam.findCcd()

        # Check getCcdExposureStatus when waiting_on_metadata is True (line 321)
        cam.user_data['waiting_on_metadata'] = True
        ready, status = cam.getCcdExposureStatus()
        assert ready is False
        assert status == 'BUSY'
        cam.user_data['waiting_on_metadata'] = False

        # Set exposure with sync=True (lines 232, 304, 307)
        sleep_counts = {'meta': 0, 'image': 0}

        def mock_sleep(sec):
            if cam.user_data['waiting_on_metadata']:
                sleep_counts['meta'] += 1
                cam.user_data['waiting_on_metadata'] = False
            elif cam.user_data['waiting_on_image']:
                sleep_counts['image'] += 1
                cam.user_data['waiting_on_image'] = False

        with patch('time.sleep', side_effect=mock_sleep), \
             patch.object(cam, '_processMetadata') as mock_meta, \
             patch.object(cam, '_queueImage') as mock_queue:
            cam.setCcdExposure(0.01, 1.0, 1, sync=True)
            assert sleep_counts['meta'] == 1
            assert sleep_counts['image'] == 1
            mock_meta.assert_called_once()
            mock_queue.assert_called_once()
