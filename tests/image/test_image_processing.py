...
def test_stretch_mode3_adaptive_mtf():
    ...
    # Test 16-bit and data.shape <= stride (stride=20)
    data_16 = np.random.randint(1000, 50000, size=(16, 16), dtype=np.uint16)
    stretched_16 = stretch_m3.stretch(data_16, image_bit_depth=16, binning=1)
    assert stretched_16.shape == (16, 16)
    assert stretched_16.dtype == np.uint16

    # Test constant array where a_max - a_min == 0 in _normalize_to_range
    const_arr = np.full((10, 10), 100.0, dtype=np.float32)
    norm = stretch_m3._normalize_to_range(const_arr, 0, 255)
    assert np.all(norm == 100.0)
