...
def test_moon_overlay_apply():
    ...
    # Test negative coordinates exceeding image width/height (lines 202 and 204: clamp to 0)
    config_neg = {
        'MOON_OVERLAY': {
            'SCALE': 0.5,
            'X': -400,
    ...
    moon_overlay_neg = IndiAllSkyMoonOverlay(config_neg)
    moon_overlay_neg.moon_orig = fake_moon.copy()
    target_img_neg = np.zeros((300, 300, 3), dtype=np.uint8)
    moon_overlay_neg.apply(target_img_neg, moon_cycle_percent=30.0, moon_phase=40.0)
    assert np.any(target_img_neg > 0)

    # Test image smaller than moon (line 178)
    small_img = np.zeros((10, 10, 3), dtype=np.uint8)
    moon_overlay_neg.apply(small_img, moon_cycle_percent=30.0, moon_phase=40.0)

    # Test lazy image loading via cv2.imread when moon_orig is None (line 45)
    moon_overlay_load = IndiAllSkyMoonOverlay(config)
    moon_overlay_load.moon_orig = None
