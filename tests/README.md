...
2. [Running Tests](#running-tests)
   ...
   - [Running Slow Tests](#running-slow-tests)
...
5. [Architecture & Testing Guidelines](#architecture--testing-guidelines)
   ...
   - [Slow Test Marking (`@pytest.mark.slow`)](#slow-test-marking-pytestmarkslow)
...
By default, [`pytest.ini`](../pytest.ini) specifies `addopts = -m "not slow"`, excluding long-running tests from everyday test runs and CI checks to maintain optimal speed.

To explicitly execute marked slow tests:

```bash
# Run ONLY slow tests
.venv/bin/python -m pytest -m "slow"

# Run slow tests for a specific subsystem
.venv/bin/python -m pytest tests/lens_solver/ -m "slow"

# Run ALL tests including both regular and slow tests
...
### Slow Test Marking (`@pytest.mark.slow`)

Tests that perform intensive numerical calculations (e.g. lens distortion fitting, high-dimensional parameter search, large array operations, or multi-iteration astrometric solves) should be marked with the `slow` marker so they do not degrade developer feedback loops or CI execution times.

#### 1. Marking a Single Test Function or Method
Use the `@pytest.mark.slow` decorator directly above the test function:
```python
import pytest

@pytest.mark.slow
def test_complex_numerical_fit():
    result = perform_heavy_optimization()
    assert result.converged is True
```

#### 2. Marking an Entire Test Class
Apply the decorator to the test class:
```python
import pytest

@pytest.mark.slow
class TestAdversarialCameraOrientation:
    def test_high_tilt_convergence(self):
        ...

    def test_extreme_distortion_recovery(self):
        ...
```

#### 3. Marking an Entire Test Module (File)
Define `pytestmark` at the module level (typically near the top of the file after imports):
```python
import pytest
...
If multiple module-level markers are needed:
```python
import pytest
