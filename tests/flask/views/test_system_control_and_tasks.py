...
def test_ajax_upload_youtube_view(flask_app, system_db):
    client = flask_app.test_client()
    ...
    with patch.dict(flask_app.config, {"LOGIN_DISABLED": True}), \
        ...
        res = client.post("/indi-allsky/ajax/uploadyoutube", json={
            "CAMERA_ID": 1,
            "VIDEO_ID": vid_id,
            "ASSET_TYPE": constants.VIDEO,
        })
        assert res.status_code == 200
        assert res.get_json()["success-message"] == "Upload task submitted"

        res_unknown = client.post("/indi-allsky/ajax/uploadyoutube", json={
            "CAMERA_ID": 1,
            "VIDEO_ID": vid_id,
            "ASSET_TYPE": 9999,
        })
        ...
        res_missing = client.post("/indi-allsky/ajax/uploadyoutube", json={
            "CAMERA_ID": 1,
            "VIDEO_ID": 999999,
            "ASSET_TYPE": constants.VIDEO,
        })
...
def test_system_control_flush_and_validate_commands(flask_app, system_db):
    ...
    with flask_app.app_context():
        ...
        vid = IndiAllSkyDbVideoTable(
            camera_id=1,
            filename="/tmp/flush_vid.mp4",
            createDate=now,
            dayDate=now.date(),
            night=False,
            success=True,
        )
        minivid = IndiAllSkyDbMiniVideoTable(
            camera_id=1,
            filename="/tmp/flush_minivid.mp4",
            createDate=now,
            dayDate=now.date(),
            night=False,
            targetDate=now,
            startDate=now,
            endDate=now,
            note="flush minivid test",
            success=True,
        )
        keogram = IndiAllSkyDbKeogramTable(
            camera_id=1,
            filename="/tmp/flush_keogram.jpg",
            createDate=now,
            dayDate=now.date(),
            night=False,
            success=True,
        )
        panovid = IndiAllSkyDbPanoramaVideoTable(
            camera_id=1,
            filename="/tmp/flush_panovid.mp4",
            createDate=now,
            dayDate=now.date(),
            night=False,
            success=True,
        )
        for row in (img, fits, raw, pano, bpm, dark, vid, minivid, keogram, panovid):
            db.session.add(row)
        db.session.commit()

    with patch.dict(flask_app.config, {"LOGIN_DISABLED": True}), \
         patch("indi_allsky.flask.views.current_user", mock_user), \
         patch.object(AjaxSystemInfoView, "verify_admin_network", return_value=True):

        res_val = client.post("/indi-allsky/ajax/system", json={
            "SERVICE_HIDDEN": "system",
            "COMMAND_HIDDEN": "validate_db",
            "CAMERA_ID": 1,
        })
        assert res_val.status_code == 200

        res_16m = client.post("/indi-allsky/ajax/system", json={
            "SERVICE_HIDDEN": "system",
            "COMMAND_HIDDEN": "flush_16min_images",
            "CAMERA_ID": 1,
        })
        assert res_16m.status_code == 200

        res_day = client.post("/indi-allsky/ajax/system", json={
            "SERVICE_HIDDEN": "system",
            "COMMAND_HIDDEN": "flush_daytime",
            "CAMERA_ID": 1,
        })
        assert res_day.status_code == 200

        res_tl = client.post("/indi-allsky/ajax/system", json={
            "SERVICE_HIDDEN": "system",
            "COMMAND_HIDDEN": "flush_timelapses",
            "CAMERA_ID": 1,
        })
        assert res_tl.status_code == 200

        res_img = client.post("/indi-allsky/ajax/system", json={
            "SERVICE_HIDDEN": "system",
            "COMMAND_HIDDEN": "flush_images",
            "CAMERA_ID": 1,
        })
        assert res_img.status_code == 200


def test_timelapse_generator_delete_actions(flask_app, system_db):
    client = flask_app.test_client()
    ...
    with flask_app.app_context():
        ...
        startrail = IndiAllSkyDbStarTrailsTable(
            camera_id=1,
            filename="/tmp/del_startrail.jpg",
            createDate=now,
            dayDate=day,
            night=True,
            success=True,
        )
        startrailvid = IndiAllSkyDbStarTrailsVideoTable(
            camera_id=1,
            filename="/tmp/del_startrailvid.mp4",
            createDate=now,
            dayDate=day,
            night=True,
            success=True,
        )
        for row in (img, vid, panovid, keogram, startrail, startrailvid):
            db.session.add(row)
        db.session.commit()

    with patch.dict(flask_app.config, {"LOGIN_DISABLED": True}), \
         patch("indi_allsky.flask.views.current_user", mock_user), \
         patch.object(AjaxTimelapseGeneratorView, "verify_admin_network", return_value=True):

        for action in ("delete_video", "delete_panorama_video", "delete_k_st", "delete_video_k_st_p"):
            payload = {
                "CAMERA_ID": 1,
