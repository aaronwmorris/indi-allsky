...
def test_ajax_focus_controller_delete_images(flask_app, focus_db):
    from datetime import date
    from indi_allsky.flask.forms import IndiAllskyTimelapseGeneratorForm
    from indi_allsky.flask.models import IndiAllSkyDbImageTable, IndiAllSkyDbPanoramaImageTable

    client = flask_app.test_client()

    with flask_app.app_context():
        cam = db.session.query(IndiAllSkyDbCameraTable).first()

        img = IndiAllSkyDbImageTable(
            filename="focus_1.jpg",
            createDate=db.func.now(),
            dayDate=date(2026, 9, 20),
            night=True,
            exposure=1.0,
            gain=100,
            adu=100,
    ...
    with patch.dict(flask_app.config, {'LOGIN_DISABLED': True}):
        with patch('indi_allsky.flask.views.current_user') as mock_user:
            mock_user.is_admin = True
            with patch.object(IndiAllskyTimelapseGeneratorForm, 'validate', autospec=True, return_value=True), \
                 patch('indi_allsky.flask.views.AjaxTimelapseGeneratorView.verify_admin_network', return_value=True):
                with patch.object(IndiAllSkyDbImageTable, 'deleteAsset', side_effect=OSError("Deletion error")), \
                     patch.object(IndiAllSkyDbPanoramaImageTable, 'deleteAsset', return_value=True):
                    res = client.post('/indi-allsky/ajax/generate?camera_id=1', json=payload)
                    assert res.status_code == 200
                    assert 'images deleted' in res.get_json()['success-message']
        ...
        with patch('indi_allsky.flask.views.current_user') as mock_user:
            ...
            with patch.object(IndiAllskyTimelapseGeneratorForm, 'validate', autospec=True, return_value=True), \
                 patch('indi_allsky.flask.views.AjaxTimelapseGeneratorView.verify_admin_network', return_value=True):
                res_inv = client.post('/indi-allsky/ajax/generate?camera_id=1', json={"CAMERA_ID": 1, "ACTION_SELECT": "invalid_action", "DAY_SELECT": "2026-09-20_night"})
