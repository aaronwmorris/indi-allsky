...
def test_ajax_image_exclude_view_branches(flask_app, gallery_db):
    ...
    # 1. Non-admin user (authenticated but not admin -> 400)
    with patch.dict(flask_app.config, {'LOGIN_DISABLED': True}), \
         patch('indi_allsky.flask.views.current_user', mock_user):
        res_nonadmin = client.post('/indi-allsky/ajax/exclude', json={'CAMERA_ID': 1, 'EXCLUDE_IMAGE_ID': img_id, 'EXCLUDE_EXCLUDE': True})
        assert res_nonadmin.status_code == 400
    ...
    with patch.dict(flask_app.config, {'LOGIN_DISABLED': True}), \
        ...
        # 2. Invalid form
        res_inv = client.post('/indi-allsky/ajax/exclude', json={'CAMERA_ID': 'invalid'})
        assert res_inv.status_code == 400

        # 3. Image not found
        res_nf = client.post('/indi-allsky/ajax/exclude', json={'CAMERA_ID': 1, 'EXCLUDE_IMAGE_ID': 99999, 'EXCLUDE_EXCLUDE': True})
        assert res_nf.status_code == 400

        # 4. Success path (exclude=True and exclude=False)
        res_ok = client.post('/indi-allsky/ajax/exclude', json={'CAMERA_ID': 1, 'EXCLUDE_IMAGE_ID': img_id, 'EXCLUDE_EXCLUDE': True})
        assert res_ok.status_code == 200
        assert res_ok.json['exclude'] is True

        res_unex = client.post('/indi-allsky/ajax/exclude', json={'CAMERA_ID': 1, 'EXCLUDE_IMAGE_ID': img_id, 'EXCLUDE_EXCLUDE': False})
        assert res_unex.status_code == 200
        assert res_unex.json['exclude'] is False
