# General
indi-allsky produces a json file every time an exposure is taken.  This file can be consumed by the telegraf agent and added to a remote InfluxDB server.

    /var/lib/indi-allsky/indi_allsky_status.json 

## Setup
Copy the file `examples/telegraf/indi_file.conf` to `/etc/telegraf/telegraf.d` and restart your telegraf daemon.
```bash
sudo cp examples/telegraf/indi_file.conf /etc/telegraf/telegraf.d/

sudo systemctl restart telegraf
```

## Dashboard template
Copy and paste this json into Grafana
```json
{
  "__inputs": [
    {
